...
- `mcd`, `ome.tiff` or `txt` data file(s) without any spaces in the file names. Associated antibody panel should contain metal and antibody information in the form of "metal_antibody" e.g. "89Y_CD45".
...
* If you use a custom module in any of the CellProfiler `cppipe` files make sure to also add it to the `plugins/cp_plugins/` directory before running the pipeline (see [Pipeline execution](#pipeline-execution)).
...
### 5. segmentation
**Input:** pre-processed full_stack of .tiff images generated by full_stack_preprocessing step and composite_Probabilities_ generated by Ilastik or composite.tiff if Ilastik is skipped.
