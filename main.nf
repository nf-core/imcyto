...
if (params.help) {
    def command = "nextflow run nf-core/imcyto --input '*_R{1,2}.fastq.gz' -profile docker"
...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'nf-core-imcyto-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/imcyto Workflow Summary'
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/imcyto] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/imcyto] Could not attach MultiQC report to summary email"
    }
